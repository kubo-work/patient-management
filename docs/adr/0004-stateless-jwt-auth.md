...
表面化した選択は 1 点に集約される。**`proxy.ts` で JWT を検証するには、`apps/web` 側にも検証手段が要る。** 現行の `jsonwebtoken` は Node の `crypto` に依存しており、この用途に流用できない。検証コードをどのライブラリで書き、どのパッケージ境界に置き、秘密鍵が無いときにどう振る舞うか——本 ADR の決定はすべてここから派生する。
